...
export default function SharedModal({
  ...
  // 모든 비디오 태그를 추적할 ref 배열
  const videoRefs = useRef<(HTMLVideoElement | null)[]>([]);

  // 인덱스 변경 시 비디오 재생/일시정지 정밀 동기화
  useEffect(() => {
    selectedEvent.assets?.forEach((_: any, index: number) => {
      ...
      if (index === currentAssetIndex) {
        ...
        // 비활성화된 영상은 일시정지 및 첫 프레임으로 이동 (로딩 버퍼 유지)
        video.pause();
        video.currentTime = 0;
      }
    });
  }, [currentAssetIndex, selectedEvent]);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative aspect-[3/4] overflow-hidden bg-stone-900">
          ...
          {/* 🌟 프리로드 최적화: 모든 에셋을 미리 렌더링하고 opacity로 전환 */}
          {selectedEvent.assets?.map((asset: any, index: number) => {
            const isVisible = index === currentAssetIndex;
            return (
              <motion.div
                ...
                style={{ zIndex: isVisible ? 1 : 0, pointerEvents: isVisible ? 'auto' : 'none' }}
              >
                {asset.asset_type === 'video' ? (
                  ...
                  <img src={asset.asset_url} alt="Memory" className="w-full h-full object-cover" />
                )}
              </motion.div>
            );
          })}
          ...
          {selectedEvent.assets?.length > 1 && (
            <div className="absolute inset-x-3 top-1/2 -translate-y-1/2 flex justify-between z-20 px-1">
              <button 
                onClick={(e) => { e.stopPropagation(); setCurrentAssetIndex(prev => (prev - 1 + selectedEvent.assets.length) % selectedEvent.assets.length); }} 
                className="p-2.5 rounded-full bg-white/80 backdrop-blur shadow-sm hover:bg-white transition-colors"
              >
                <ChevronLeft size={18} className="text-stone-800" />
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); setCurrentAssetIndex(prev => (prev + 1) % selectedEvent.assets.length); }} 
                className="p-2.5 rounded-full bg-white/80 backdrop-blur shadow-sm hover:bg-white transition-colors"
              >
          ...
          <button 
            onClick={() => setSelectedEvent(null)} 
            className="absolute top-5 right-5 p-2 bg-black/40 text-white backdrop-blur-md rounded-full z-20 hover:bg-black/60 transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-6 lg:p-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-extrabold text-stone-400 uppercase tracking-[0.2em]">
          ...
          <div className="pt-5 border-t border-stone-100/60 flex flex-col gap-5">
            ...
            <div className="flex justify-between items-center bg-stone-50 p-1.5 rounded-2xl">
              <button 
                ...
                disabled={timeline.findIndex((e: any) => e.id === selectedEvent.id) === 0} 
                className="flex items-center gap-2 px-5 py-2 text-stone-400 disabled:opacity-20 hover:text-stone-800 transition-colors"
              >
              ...
              <button 
                ...
                disabled={timeline.findIndex((e: any) => e.id === selectedEvent.id) === timeline.length - 1} 
                className="flex items-center gap-2 px-5 py-2 text-stone-400 disabled:opacity-20 hover:text-stone-800 transition-colors"
              >
